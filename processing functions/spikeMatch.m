function select_spike_match = spikeMatch(select_all, threshQ2, threshQ4, g, coeffs, folderSpikeMatch, basename, min_coeff_count, corr_thresh)
    ...
    hmap = figure;
    heatmap(overlapMatFull, 'ColorLimits', [0, 1], 'Title', sprintf('Pairwise Spike Overlap: %s', basename), ...
    ...
    saveas(hmap, fullfile(folderSpikeMatch, ['overlapFll_' basename '.png']), 'png');
  %  close(hmap);
    % visualize the overlap row correlation
    hCorr = figure;
    overlapMatFull = overlapMatFull + overlapMatFull' + eye(n);

    heatmap(overlapMatFull, 'ColorLimits', [-1, 1], 'Title', sprintf('Overlap Row Correlation (Skipping Zeros/Same Gaussians): %s', basename), ...
             'XLabel', 'Coefficient (C) and Gaussian (G) ID', ...
             'YLabel', 'Coefficient (C) and Gaussian (G) ID', ...
             'XDisplayLabels', heatmap_labels, ...
             'YDisplayLabels', heatmap_labels);
    saveas(hCorr, fullfile(folderSpikeMatch, ['correlation_overlap_Full' basename '.png']), 'png');
    close(hCorr);
